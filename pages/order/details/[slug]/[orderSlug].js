...
const OrderDetails = (props) => {
  ...
  // const isBundle = orderDetails?.bundle ? true : false;
  // const bundleInfo = orderDetails?.bundle_details;
  // const isChild = orderDetails?.children ? true : false;
  // const childInfo = orderDetails?.children || [];
  ...
  const switchOrderBundle = (nft_slug, order_slug) => {
    if (slug !== nft_slug)
      router.push(`/order/details/${nft_slug}/${order_slug}`);
  ...
  return (
    <>
      {props?.nft?.name && (
        <Header
          bgImage
          title={`${props?.nft?.name} | Jump.trade`}
          description={`${props?.nft?.name} is a Meta Cricket League ${props?.nft?.core_statistics?.role?.value} Playable NFT! Purchase This NFT Now to Play the MCL P2E Cricket Game & Win Cash Rewards!`}
          image={
            props?.nft?.asset_type?.includes("image")
              ? props?.nft?.asset_url
              : props?.nft?.cover_url
          }
          canonical={`${process.env.NEXT_PUBLIC_MARKETPLACE_URL}/order/details/${props?.nft?.slug}/${props?.nft?.order_details?.slug}`}
          recentSoldContent={true}
        />
      )}
      {loader ? (
        <NFTLoader />
            ...
            <div className="container-fluid">
              ...
              <div className="row mt-5">
                ...
                <div className="col-12 col-lg-6 order-lg-1 order-2">
                  ...
                  <ChainAttributes nft={nft} chains={nft.chain_attributes} />

                  {nft?.core_statistics?.role?.value !== "Shot" &&
                    nft?.core_statistics?.role?.value !== "Car" &&
                    nft?.core_statistics?.role?.value !== "Land" &&
                    nft?.core_statistics?.role?.value !== "Building" &&
                    nft?.core_statistics?.role?.value !== "Fielder" &&
                    nft?.core_statistics &&
                    nft?.statistics.length > 0 && (
                      <>
                        <div className="mt-5"></div>
                        <NFTPlayerStats
                          nftData={nft}
...
export async function getServerSideProps({ query }) {
  ...
  try {
    ...
    const NFT = response?.data?.data?.nft;
    props.nft = NFT;
    props.erc721 = NFT.nft_type === "erc721";

    if (NFT.celebrity_slug) {
      let response = await artistApi(NFT.celebrity_slug);
      props.artist = response?.data?.data?.celebrity;
    }

    if (NFT?.order_details) {
      if (NFT?.order_details?.timed_auction) {
        props.auctionEndTime = NFT.order_details?.auction_end_time;
        props.isAuctionStarted =
          new Date(NFT.time).getTime() >=
          new Date(NFT.order_details?.auction_start_time).getTime();
      ...
      props.isOrderOnSale = NFT.order_details?.status === "onsale";
      props.isOrderSuccess = NFT.order_details?.status === "success";
      props.isOrderCancelled = NFT.order_details?.status === "cancelled";

      if (NFT?.order_details?.purchase_details.length > 0) {
        props.purchaseList = NFT?.order_details?.purchase_details;
      }
    }
    ...
    if (
      ...
      props.transferringNFT = true;
    }

    if (NFT.nft_type === "erc721" && orderSlug) {
      let history = await orderBidHistory({ order_slug: orderSlug, page: 1 });
      props.BidHistory = history.data.data.histories;
      props.totalCount = history.data.data.total_count;
    }
  } catch (err) {
    // console.log("Error", err?.status || err?.response?.status);
