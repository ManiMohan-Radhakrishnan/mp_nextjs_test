  ...
  const handleSubscribe = async () => {
    if (validateEmail(email)) {
      try {
        ...
        const response = await subscribeApi(email, termsConditions, "jump");
      ...
      <div className={style["join-waitlist-form-block"]}>
        ...
        {autoLogin && email && !loginStatus && (
          <div className={`${style["getappLinkCheckbox"]} py-2`}>
            ...
            />{" "}
            I allow Jump.trade to create an account for me and I confirm that I
            am 18 years or older. View{" "}
            <a
              target="_blank"
              onClick={() =>
                window.open(
