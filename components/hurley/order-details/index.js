...
import NFTOrderSummary from "../nft-order-summary";
import NFTOrderBaseDetails from "../nft-order-basic-details";
import NFTPurchaseDetails from "../nft-purchase-details/index";
import BidWinner from "../bid-winner/index";
...
const OrderDetails = ({ CurrentOrderSlug, details = false, nftData }) => {
  const dispatch = useDispatch();
  const [bidExpiry, setBidExpiry] = useState();
  const [isBidder, setIsBidder] = useState(false);
  const router = useRouter();
  let { slug, orderSlug } = router.query;
  ...
  const [trackEventOnce, setTrackEventOnce] = useState(false);

  const nftType = nftData?.core_statistics?.role?.value;

  useEffect(() => {
    if (Object.keys(nft).length > 0 && nftOwner.length > 0 && !trackEventOnce) {
      invokeTrackEvent(EVENT_NAMES?.NFT_VIEWED, {
        name: nftData?.name,
        slug: nftData?.slug,
        Role: nftData?.core_statistics?.role?.value,
        Category: nftData?.core_statistics?.category?.value,
        Level: nftData?.core_statistics?.level?.value
          ? nftData?.core_statistics?.level?.value
          : null,
        "Sale Type":
          nft?.order_details?.is_bid && nft?.order_details?.is_buy
            ...
            ? "Bid"
            : null,
        "Owned By": nftOwner[0]?.user_name ? nftOwner[0].user_name : null,
        "Minimum Bid": nft?.order_details?.minimum_bid
          ? parseFloat(nft?.order_details?.minimum_bid)
          : null,
        "Limited Edition": null,
        "End of Auction": nft?.order_details?.auction_end_time
          ? new Date(nft?.order_details?.auction_end_time)
          : null,
        "Start of Auction": nft?.order_details?.auction_start_time
          ? new Date(nft?.order_details?.auction_start_time)
          : null,
      });
      setTrackEventOnce(true);
    }
    ...
    <>
      {nftData?.name && (
        <Header
          ...
          description={`${nftData?.name} is a Meta Cricket League ${nftType} Playable NFT! Purchase This NFT Now to Play the MCL P2E Cricket Game & Win Cash Rewards!`}
          image={
            nftData?.asset_type?.includes("image")
              ? nftData?.asset_url
              : nftData?.cover_url
          }
          canonical={`${process.env.NEXT_PUBLIC_MARKETPLACE_URL}/nft-marketplace/details/${nftData?.slug}`}
        />
      )}
