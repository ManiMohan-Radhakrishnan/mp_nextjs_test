  ...
  return (
    <>
      ...
      <div className={`creator mt-3 mb-3`}>
        ...
        //     return router.push(
        //       `/nft-marketplace/cricket-shot-nfts/${nft?.category_slug}`
        //     );
        //   if (nft?.core_statistics?.role?.value === "Fusor")
        //     return router.push(
        //       `/nft-marketplace/cricket-fusor-nfts/${nft?.category_slug}`
        //     );
        //}}
        >
          {nft?.category_name}
      ...
      {nft?.core_statistics?.role?.value === "Shot" && (
        <>
          ...
          <div className="bottom-content-box">
            <ShotPitchStatsDirection ShotsData={nft?.core_statistics} />
          </div>
        </>
      )}
      ...
      {nft?.core_statistics?.role?.value === "Ball" && (
        <>
          ...
          <div className="bottom-content-box">
            <BallNftStatsDetails
              ballAttributes={nft?.core_statistics?.ability_list?.value}
            />
          </div>
        </>
      )}
      ...
      {nft?.core_statistics?.role?.value === "Fielder" && (
        <>
          <hr className="custom-divider dashed-divider" />
          <div className="bottom-content-box">
            <FieldingPitchStatsDirection FieldingData={nft?.statistics[0]} />
          </div>
        </>
      )}
            ...
            {bundleInfo.map((bundle, i) => {
              return (
                <li
                  ...
                  onClick={() => switchOrderBundle(bundle?.nft_slug, orderSlug)}
                >
              ...
              {childInfo.map((child, i) => {
                return (
                  <li
                    ...
                    onClick={() => switchOrderBundle(child?.slug, orderSlug)}
                  >
                  ...
                  return (
                    <ToolTip
                      icon={
                        <button
                          disabled={true}
                          className={`btn btn-dark text-center btn-lg mt-2 place-bid-btn w-100`}
                        >
                          Token Transfer Initiated{" "}
                          <BsFillQuestionCircleFill
                            size={16}
                              ...
                              onClick={() => {
                                if (!inCart) {
                                  invokeTrackEvent(EVENT_NAMES?.ADDED_TO_CART, {
                                    ...
                                    "End of Auction": nft?.order_details
                                      ?.auction_end_time
                                      ? new Date(
                      ...
                      {User?.kyc_status !== "success" ? (
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            invokeTrackEvent(EVENT_NAMES.CHECKOUT_STARTED, {
                              Name: nft?.name,
                              "Total Amount": orderDetails?.buy_amount
                                ? parseFloat(orderDetails?.buy_amount)
                                : null,
                              "Cash Rewards Points": parseInt(
                                User?.assert_balances?.reward_point
                              ),
                              "JT Points": User?.assert_balances?.jump_point,
                              "Buy Price": orderDetails?.buy_amount
                                ? parseFloat(orderDetails?.buy_amount)
                                : null,
                        ...
                        onClick={() => {
                          ...
                          invokeTrackEvent(EVENT_NAMES.CHECKOUT_STARTED, {
                            ...
                            "Total Amount": orderDetails?.buy_amount
                              ? parseFloat(orderDetails?.buy_amount)
                              : null,
                            "Cash Rewards Points": User?.assert_balances
                              ?.reward_point
                              ? parseInt(User?.assert_balances?.reward_point)
                              : 0,
                            "JT Points": User?.assert_balances?.jump_point,
                            "Buy Price": orderDetails?.buy_amount
                              ? parseFloat(orderDetails?.buy_amount)
