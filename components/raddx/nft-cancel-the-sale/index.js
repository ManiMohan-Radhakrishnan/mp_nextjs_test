...
const NFTCancelTheSale = ({
  ...
  const isBundle = nft?.order_details?.bundle_details ? true : false;
  ...
  const handleSaleCancel = async () => {
    try {
      let result = {};
      if (nft?.order_details?.bundle) {
        result = await bundleSaleCancelApi({
          bundle_id: nft?.order_details?.bundle_id,
        });
      } else
        result = await saleCancelApi({
                    ...
                    {isBundle ? (
                      <>
                        <div className={`sucess-title cancel-confirm`}>
                          {bundleInfo.map((nft = {}, i) => {
                            return (
                              <div
                                className={"pop-nft-info"}
                                key={nft?.id || i}
                              >
                                <div className={"pop-nft-media"}>
                                  {(() => {
                                ...
                                <div className={"pop-nft-content"}>
                                  ...
                                  <div
                                    className={`pop-nft-title text-center mb-1`}
                                  >
                                    {nft?.name}
                                  </div>
                                  {/* <div className={`erc-type text-center mb-1`}>
